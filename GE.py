...
# ------------------------------------------------------------
# Tư tưởng giữ nguyên:
# - Vẫn dùng ellipsoid (GE) để biểu diễn dữ liệu
# - Vẫn split ellipsoid theo trục chính
# - Vẫn refine bằng khoảng cách Mahalanobis
# - Vẫn tính density, min-dist, gamma
# - Vẫn phân cụm theo tinh thần DPC
#
# Khác biệt chính của hướng phát triển:
# - Không dùng full covariance inverse như code gốc
# - Dùng low-rank approximation + randomized SVD
# - Dùng công thức nhanh để giảm chi phí khi số chiều lớn
#
# Quy ước comment:
# - [GỐC] = hàm giữ tinh thần / logic gần code gốc
# - [MỚI] = hàm thêm mới cho hướng phát triển high-dimensional
# ============================================================


# ============================================================
# CẤU HÌNH NỘI BỘ CỐ ĐỊNH CHO HƯỚNG PHÁT TRIỂN
# ------------------------------------------------------------
# 3 tham số này là tham số kỹ thuật nội bộ.
# Theo đúng định hướng bạn chọn: KHÔNG tune, KHÔNG đưa ra ngoài.
# ============================================================
INTERNAL_APPROX_RANK = 16
INTERNAL_SVD_OVERSAMPLES = 8
INTERNAL_SVD_N_ITER = 2


# ============================================================
# [MỚI] Lớp ellipsoid tăng tốc cho dữ liệu nhiều chiều
# ------------------------------------------------------------
# So với code gốc:
# - code gốc dùng full covariance matrix + inverse đầy đủ
# - bản này dùng low-rank shape model để giảm chi phí tính toán
# ============================================================
class FastEllipsoid:
    def __init__(self, data, indices, epsilon=1e-6):
        """
        [MỚI]
        Khởi tạo một ellipsoid tăng tốc cho dữ liệu nhiều chiều.

        Tham số:
        - data: ma trận dữ liệu thuộc ellipsoid, shape (n_samples, d)
        - indices: chỉ số các điểm trong dữ liệu gốc
        - epsilon: tham số gốc của thuật toán, giúp ổn định số học

        Khác với code gốc:
        - code gốc dùng full covariance + inverse
        - bản này dùng low-rank approximation bên trong
        """
        ...
        # 3 tham số nội bộ cố định, không tune
        self.approx_rank = INTERNAL_APPROX_RANK
        self.oversamples = INTERNAL_SVD_OVERSAMPLES
        self.n_iter = INTERNAL_SVD_N_ITER
    ...
    def _fit_low_rank_shape(self):
        """
        [MỚI]
        Xấp xỉ shape/covariance của ellipsoid bằng low-rank SVD.

        Mục tiêu:
        - giữ tinh thần ellipsoid của GE-DPC
        - nhưng giảm chi phí tính toán khi dữ liệu nhiều chiều

        Ý tưởng:
        - Trừ tâm dữ liệu
        - Chạy randomized SVD
        - Chỉ giữ các hướng quan trọng nhất
        - Dựng phổ xấp xỉ cho ma trận H = S + epsilon * I
        """
        Xc = self.data - self.center
        d = self.dim

        # Trường hợp dữ liệu quá ít hoặc gần như không có phương sai
        ...
        try:
            ...
            # Nếu randomized SVD lỗi thì fallback về SVD thường
            _, s, Vt = np.linalg.svd(Xc, full_matrices=False)
            s = s[:max_rank]
            U_feat = Vt[:max_rank].T

        # singular values -> covariance eigenvalues
        cov_eigs = (s ** 2) / max(self.n_samples, 1)

        # Chỉ giữ các thành phần có ý nghĩa
        keep = cov_eigs > 1e-12
        self.U = U_feat[:, keep]
        self.cov_eigs = cov_eigs[keep]
        self.rank = len(self.cov_eigs)

        # Dùng cho tính Mahalanobis nhanh
        self._inv_diag = 1.0 / (self.cov_eigs + self.epsilon)

        # Phổ xấp xỉ của H = S + epsilon I
    ...
    def mahal_sq_points(self, points):
        ...
        Khác code gốc:
        - code gốc dùng inv_H đầy đủ
        - bản này dùng công thức low-rank nhanh hơn
        """
        X = np.asarray(points, dtype=float) - self.center
        if X.ndim == 1:
            X = X[None, :]

        # Nếu rank = 0 thì gần như ellipsoid tròn rất nhỏ
        if self.rank == 0:
            return np.sum(X * X, axis=1) / self.epsilon

        # Thành phần theo các hướng chính
        proj = X @ self.U
        parallel_sq = np.sum((proj ** 2) * self._inv_diag[None, :], axis=1)

        # Thành phần vuông góc còn lại
    ...
    def _compute_rho(self):
        """
        [GẦN GỐC]
        Tính rho của ellipsoid = khoảng cách Mahalanobis lớn nhất từ tâm đến các điểm.

        Ý tưởng giống code gốc, chỉ khác ở chỗ khoảng cách Mahalanobis
        được tính bằng công thức tăng tốc.
        """
    ...
    def _compute_lengths(self):
        ...
        Ý tưởng giống code gốc:
        - semi-axis length = rho * sqrt(eigenvalues(H))
        """
    ...
    def compute_major_axis_endpoints(self):
        """
        [GỐC]
        Tìm 2 đầu mút gần đúng của trục chính dài nhất để phục vụ split.

        Logic giữ rất sát code gốc:
        - lấy điểm gần tâm
        - tìm điểm xa nhất với nó
        - rồi tìm tiếp điểm xa nhất nữa
        """
        if self.n_samples <= 1:
...
def get_num(ellipsoid):
    """
    [GỐC]
    Trả về số điểm trong một ellipsoid.
    """
...
def calculate_ellipsoid_density(ellipsoid):
    """
    [GỐC]
    Tính mật độ của một ellipsoid.

    Ý tưởng giữ nguyên tinh thần code gốc:
    density = (n_samples^2) / (tổng độ dài trục * tổng khoảng cách Mahalanobis)
    """
...
def splits(ellipsoid_list, num):
    ...
    Nếu ellipsoid có số điểm < num thì giữ nguyên.
    Nếu >= num thì tiếp tục split.
    """
    new_ells = []
...
def splits_ellipsoid(ellipsoid):
    ...
    Logic giữ sát code gốc:
    1. Chia thô theo khoảng cách tới 2 đầu mút trục chính
    2. Tạo 2 ellipsoid tạm
    3. Refine lại bằng khoảng cách Mahalanobis
    ...
    # Bước chia thô ban đầu theo khoảng cách Euclidean tới 2 đầu mút
    ...
    # Refinement Mahalanobis - đúng tinh thần bài gốc
...
def recursive_split_outlier_detection(initial_ellipsoids, data, t=2.0, max_iterations=10):
    """
    [GỐC]
    Tách thêm các ellipsoid ngoại lệ.

    Ý tưởng giống code gốc:
    - tìm ellipsoid có tổng trục quá lớn
    - thử split tiếp
    - nếu mật độ sau split tăng đủ tốt thì chấp nhận
    """
    ...
    for _ in range(max_iterations):
        ...
        # giữ đúng tinh thần ngưỡng lá nhỏ như code gốc
...
def _pair_distance_fast(ell_i, ell_j):
    ...
    Đây là phần mới của hướng phát triển:
    - thay vì dùng full inverse của trung bình covariance
    - dùng xấp xỉ low-rank để tính nhanh hơn
    """
...
def ellipse_distance(ellipsoid_list):
    ...
    Ý tưởng giống code gốc.
    Khác biệt:
    - bên trong dùng hàm khoảng cách nhanh _pair_distance_fast()
    """
...
def ellipse_min_dist(dist_mat, densities):
    """
    [GỐC]
    Với mỗi ellipsoid, tìm khoảng cách nhỏ nhất đến ellipsoid có density lớn hơn.

    Đây là bước delta/min-dist rất quan trọng của DPC.
    """
...
def auto_select_centers(densities, min_dists, mode='knee', top_k=None, min_centers=1, max_centers=None):
    """
    [KHÔNG PHẢI CODE GỐC NGUYÊN BẢN]
    Hàm tự động chọn center thay cho thao tác chọn thủ công trên decision graph.

    Đây là phần bạn đã thêm từ trước để chạy batch/auto.
    Vẫn giữ nguyên vì bạn đang dùng 4 tham số center này làm tham số chính.

    mode:
    - 'knee': chọn theo điểm gãy trên đường gamma
    - 'threshold': chọn theo ngưỡng mean + std
    - top_k: nếu không None thì lấy đúng top-k
    """
    densities = np.asarray(densities, dtype=float)
...
def ellipse_cluster(densities, centers, nearest):
    """
    [GỐC]
    Gán nhãn cụm cho từng ellipsoid theo quy tắc DPC:
    - center có nhãn trước
    - ellipsoid còn lại nhận nhãn của ellipsoid gần nhất có density cao hơn
    """
...
def align_labels(true_labels, pred_labels):
    """
    [PHỤ TRỢ ĐÁNH GIÁ]
    Căn chỉnh nhãn clustering với nhãn thật trước khi tính ACC.

    Vì clustering có thể cho nhãn 0,1,2... khác thứ tự nhãn thật,
    nên cần Hungarian matching để map lại cho đúng.
    """
...
def run_ge_dpc_highdim(
    ...
    Theo đúng định hướng bạn chốt:
    - CHỈ còn 6 tham số chính
    - 3 tham số high-dimensional là nội bộ cố định, không đưa ra ngoài
    ...
    # Giữ đúng tinh thần code gốc:
    # num = ceil(sqrt(n))
    num = int(np.ceil(np.sqrt(data.shape[0])))

    # ========================================================
    # 1) Sinh ellipsoid ban đầu và safe split
    # ========================================================
    ...
    # ========================================================
    # 2) Split thêm các ellipsoid ngoại lệ
    # ========================================================
    ...
    print(
        f"A total of {len(ellipsoid_list)} ellipsoids were generated "
        f"(Tổng cộng đã tạo {len(ellipsoid_list)} ellipsoid)"
    )
    ...
    # ========================================================
    # 3) Tính thuộc tính ellipsoid: density, dist, min-dist
    # ========================================================
    ...
    # ========================================================
    # 6) Ánh xạ nhãn ellipsoid về từng điểm dữ liệu
    # ========================================================
...
if __name__ == "__main__":
    ...
    # ========================================================
    # 6 THAM SỐ CHÍNH CỦA THUẬT TOÁN
    # --------------------------------------------------------
    # 2 tham số thuật toán gốc:
    # - epsilon
    # - outlier_t
    #
    # 4 tham số chọn center:
    # - auto_center_mode
    # - auto_center_k
    # - min_centers
    # - max_centers
