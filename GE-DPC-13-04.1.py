...
# Fully automatic center-selection + reduced-parameter version
#
# Main ideas:
...
# 5) center selection is fully automatic:
#       - gamma sorting
#       - largest relative drop
#       - redundancy pruning
#       - second-center fallback
# ============================================================
...
# def auto_select_centers(densities, min_dists, dist_matrix):
#     """
#     Fully automatic center selection.

#     Strategy:
#     1) gamma = density * min_dist
#     2) sort gamma descending
#     3) find largest relative drop
#     4) select top-k automatically
#     5) prune redundant centers by distance
#     6) add a second center if the first collapse is too aggressive
#     """
#     densities = np.asarray(densities, dtype=float)
...
#     if np.allclose(g_sorted, g_sorted[0]):
#         return [int(order[0])]

#     eps = 1e-12

#     # 1) largest relative drop
#     rel_drops = (g_sorted[:-1] - g_sorted[1:]) / (np.abs(g_sorted[:-1]) + eps)
#     rel_drops = np.maximum(rel_drops, 0.0)

#     best_idx = int(np.argmax(rel_drops))
#     k_auto = best_idx + 1

#     # 2) conservative fallback when head is too flat
#     max_drop = float(rel_drops[best_idx]) if len(rel_drops) > 0 else 0.0
#     if max_drop < 0.10:
#         g_med = float(np.median(g_sorted))
#         k_prom = int(np.sum(g_sorted > g_med))
#         if k_prom > 0:
#             k_auto = min(k_auto, k_prom)
#         k_auto = max(1, k_auto)

#     k_auto = max(1, min(k_auto, n))
#     candidate_centers = order[:k_auto].tolist()

#     # 3) redundancy pruning
#     positive_deltas = min_dists[min_dists > 0]
#     tau = float(np.median(positive_deltas)) if len(positive_deltas) > 0 else 0.0

#     pruned = []
#     for c in candidate_centers:
#         keep = True
#         for kept in pruned:
#             if dist_matrix[c, kept] <= tau:
#                 keep = False
#                 break
#         if keep:
#             pruned.append(c)

#     candidate_centers = pruned

#     # 4) second-center fallback
#     if len(candidate_centers) == 1 and n >= 2:
#         c1 = int(order[0])
#         c2 = int(order[1])
...
#         tau_far = float(np.median(positive_deltas)) if len(positive_deltas) > 0 else 0.0

#         if ratio_2 >= 0.5 and dist_matrix[c1, c2] > tau_far:
#             candidate_centers.append(c2)

#     if len(candidate_centers) == 0:
#         candidate_centers = [int(order[0])]
...
#     1) gamma = density * min_dist
#     2) sort gamma descending
#     3) detect a prominent head automatically
#     4) choose k inside the head using relative-drop rule
#     5) prune redundant centers by distance
#     6) fallback for missing second center
#     """
#     densities = np.asarray(densities, dtype=float)
#     min_dists = np.asarray(min_dists, dtype=float)
#     dist_matrix = np.asarray(dist_matrix, dtype=float)
...
#     # ---------------------------------------------------------
#     # Step 1: prominent head by relative strength
#     # ---------------------------------------------------------
#     prominence_ratio = 0.20
#     strong_mask = g_sorted >= prominence_ratio * g1
#     strong_idx = np.where(strong_mask)[0]

#     if len(strong_idx) == 0:
#         head_len = 1
#     else:
#         head_len = int(strong_idx[-1]) + 1

#     head_len = max(1, min(head_len, n))

#     # ---------------------------------------------------------
#     # Step 2: choose k inside head
#     # ---------------------------------------------------------
#     if head_len == 1:
#         k_auto = 1
#     else:
#         g_head = g_sorted[:head_len]
#         rel_drops = (g_head[:-1] - g_head[1:]) / (np.abs(g_head[:-1]) + eps)
#         rel_drops = np.maximum(rel_drops, 0.0)
...
#         # if the last candidate in the head is still reasonably strong,
#         # keep the whole head.
#         last_head_ratio = float(g_head[-1] / (g1 + eps))

#         if last_head_ratio >= 0.20:
#             k_auto = head_len
#         else:
#             k_auto = k_drop
...
def auto_select_centers(densities, min_dists, dist_matrix):
    """
    Fully automatic center selection using:
    - multi-candidate generation
    - internal scoring
    - redundancy pruning

    Main idea:
    1) Build several candidate center sets from different heuristics
    2) For each candidate set, simulate ellipsoid clustering
    3) Score candidates by internal quality:
       - higher separation is better
       - lower compactness is better
       - fewer tiny clusters is better
       - too many centers are penalized
    4) Return the best candidate set
    """
    densities = np.asarray(densities, dtype=float)
    min_dists = np.asarray(min_dists, dtype=float)
    dist_matrix = np.asarray(dist_matrix, dtype=float)
    ...
    # ---------------------------------------------------------
    # helper: redundancy pruning
    # ---------------------------------------------------------
    positive_deltas = min_dists[min_dists > 0]
    tau = float(np.median(positive_deltas)) if len(positive_deltas) > 0 else 0.0

    def prune_redundant(center_list):
        ...
        return pruned if len(pruned) > 0 else [center_list[0]]

    # ---------------------------------------------------------
    # helper: build nearest-higher array from densities
    # ---------------------------------------------------------
    def compute_nearest_higher():
        nearest = -np.ones(n, dtype=int)
        density_order = np.argsort(-densities)

        for idx in density_order[1:]:
            mask = densities > densities[idx]
            if np.any(mask):
                cand = np.where(mask)[0]
                nearest[idx] = cand[np.argmin(dist_matrix[idx, cand])]

        return nearest

    nearest_higher = compute_nearest_higher()

    # ---------------------------------------------------------
    # helper: simulate clustering for candidate centers
    # ---------------------------------------------------------
    def simulate_cluster(candidate_centers):
        ...
        # assign remaining unassigned to nearest selected center
        if np.any(labels == -1):
            for idx in np.where(labels == -1)[0]:
                nearest_center_local = np.argmin(dist_matrix[idx, candidate_centers])
                labels[idx] = nearest_center_local

        return labels

    # ---------------------------------------------------------
    # helper: internal scoring
    # ---------------------------------------------------------
    def score_candidate(candidate_centers):
        candidate_centers = list(candidate_centers)
        k = len(candidate_centers)
        if k == 0:
            return -np.inf

        labels = simulate_cluster(candidate_centers)

        # separation: mean pairwise center distance
        if k == 1:
            separation = 0.0
        else:
            pair_dists = []
            for i in range(k):
                for j in range(i + 1, k):
                    pair_dists.append(dist_matrix[candidate_centers[i], candidate_centers[j]])
            separation = float(np.mean(pair_dists)) if len(pair_dists) > 0 else 0.0

        # compactness: mean distance from ellipsoid to its assigned center
        compactness_vals = []
        for idx in range(n):
            c_idx = candidate_centers[labels[idx]]
            compactness_vals.append(dist_matrix[idx, c_idx])
        compactness = float(np.mean(compactness_vals)) if len(compactness_vals) > 0 else np.inf

        # small-cluster penalty
        cluster_sizes = np.array([np.sum(labels == i) for i in range(k)], dtype=float)
        small_cluster_penalty = float(np.mean(cluster_sizes <= 1)) if len(cluster_sizes) > 0 else 0.0

        # imbalance penalty
        if len(cluster_sizes) > 1 and np.mean(cluster_sizes) > 0:
            imbalance_penalty = float(np.std(cluster_sizes) / (np.mean(cluster_sizes) + eps))
        else:
            imbalance_penalty = 0.0

        # too-many-centers penalty
        center_penalty = float(k / max(n, 1))

        # reward explained gamma mass
        gamma_mass = float(np.sum(gamma[candidate_centers]) / (np.sum(gamma) + eps))

        # final score
        score = (
            (separation / (compactness + eps))
            + 0.35 * gamma_mass
            - 0.60 * small_cluster_penalty
    ...
    # ---------------------------------------------------------
    # Candidate A: largest relative drop
    # ---------------------------------------------------------
    rel_drops = (g_sorted[:-1] - g_sorted[1:]) / (np.abs(g_sorted[:-1]) + eps)
    rel_drops = np.maximum(rel_drops, 0.0)
    ...
    best_idx = int(np.argmax(rel_drops))
    k_drop = best_idx + 1
    cand_drop = prune_redundant(order[:max(1, k_drop)].tolist())

    # ---------------------------------------------------------
    # Candidate B: prominent head
    # ---------------------------------------------------------
    prominence_ratio = 0.20
    strong_idx = np.where(g_sorted >= prominence_ratio * g_sorted[0])[0]
    head_len = int(strong_idx[-1]) + 1 if len(strong_idx) > 0 else 1
    cand_head = prune_redundant(order[:max(1, head_len)].tolist())

    # ---------------------------------------------------------
    # Candidate C: normalized gamma threshold
    # ---------------------------------------------------------
    g_norm = (g_sorted - np.min(g_sorted)) / (np.max(g_sorted) - np.min(g_sorted) + eps)
    thr = float(np.mean(g_norm) + 0.5 * np.std(g_norm))
    idx_thr = np.where(g_norm >= thr)[0]
    k_thr = int(idx_thr[-1]) + 1 if len(idx_thr) > 0 else 1
    cand_thr = prune_redundant(order[:max(1, k_thr)].tolist())

    # ---------------------------------------------------------
    # Candidate D: elbow on gamma curve
    # ---------------------------------------------------------
    if n == 2:
        k_elbow = 1
    else:
        ...
        x_norm = x / max(n - 1, 1)
        y_norm = (g_sorted - np.min(g_sorted)) / (np.max(g_sorted) - np.min(g_sorted) + eps)
    ...
    cand_elbow = prune_redundant(order[:max(1, k_elbow)].tolist())

    # ---------------------------------------------------------
    # Candidate E: fallback top-2 if second is still strong
    # ---------------------------------------------------------
    cand_two = [int(order[0])]
    if n >= 2:
        ratio_2 = float(g_sorted[1] / (g_sorted[0] + eps))
        tau_far = tau
        if ratio_2 >= 0.5 and dist_matrix[order[0], order[1]] > tau_far:
            cand_two = prune_redundant([int(order[0]), int(order[1])])

    # ---------------------------------------------------------
    # collect unique candidates
    # ---------------------------------------------------------
    raw_candidates = [cand_drop, cand_head, cand_thr, cand_elbow, cand_two]
    ...
    for cand in raw_candidates:
        ...
        if cand not in seen:
            seen.add(cand)
            unique_candidates.append(list(cand))
    ...
    if best_centers is None or len(best_centers) == 0:
        best_centers = [int(order[0])]
...
def summarize_rank_info(ellipsoid_list):
    ...
    print("- Auto rank rule: rank = min(d, n_samples, max(6, ceil(2 * sqrt(d))))")
    print(f"- Rank min / avg / max: {np.min(ranks)} / {np.mean(ranks):.2f} / {np.max(ranks)}")
    print(f"- SVD modes used: {svd_modes}")


def summarize_center_info(densities, min_dists, selected):
    gamma = np.asarray(densities, dtype=float) * np.asarray(min_dists, dtype=float)
    order = np.argsort(-gamma)
    g_sorted = gamma[order]

    print("Automatic center-selection summary (Tóm tắt chọn center tự động):")
    print("- Strategy: gamma sorting + largest relative drop + redundancy pruning + fallback")
    ...
    if len(g_sorted) > 1:
        rel_drops = (g_sorted[:-1] - g_sorted[1:]) / (np.abs(g_sorted[:-1]) + 1e-12)
        print(f"- Max relative drop: {float(np.max(rel_drops)):.6f}")
...
def run_ge_dpc_highdim_fast(
    ...
    print("Center mode: fully automatic (no top_k, no min_centers, no max_centers)")
    ...
    summarize_center_info(densities, min_dists, selected)
    ...
    return {
        ...
        'n_selected_centers': len(selected),
