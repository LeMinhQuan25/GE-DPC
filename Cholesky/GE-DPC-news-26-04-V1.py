...
def is_distribution_acceptable(
    ...
    if max_largest_ratio is None:
        max_largest_ratio = 0.90 if expected_k <= 2 else 0.72
...
def internal_cluster_score(
    ...
    smallest_ratio = float(np.min(sizes) / np.sum(sizes))

    # Stronger distribution penalty than previous version.
    if expected_k <= 2:
        if largest_ratio > 0.90 or smallest_ratio < 0.01:
            return -1e18
    else:
        if largest_ratio > 0.72 or smallest_ratio < 0.025:
            return -1e18

    imbalance = largest_ratio - (1.0 / expected_k)
    # imbalance_penalty = 1.0 / (1.0 + 4.0 * max(imbalance, 0.0))
