...
# ============================================================
# GE-DPC with Uniform Grid Initialization + Cholesky + Cache
# ------------------------------------------------------------
# Mục tiêu của bản này:
# 1) Giữ lõi GE-DPC hiện tại của bạn:
#    - Ellipsoid geometry
#    - safe split
#    - Mahalanobis refinement
#    - outlier-detection split
#    - GE-level DPC clustering
# 2) Bổ sung giai đoạn đầu theo pipeline đề xuất:
#    Input -> Grid initialization -> Local seed generation
#    -> GE generation -> Split/refine -> GE-DPC clustering -> Output
# 3) Không dùng approximate matrix computation / randomized SVD.
# 4) Không dùng nghịch đảo ma trận tường minh; dùng Cholesky + solve.
#
# Ghi chú rất quan trọng:
# - Grid trong file này chỉ đóng vai trò PHÂN HOẠCH THÔ / KIỂM SOÁT CỤC BỘ.
# - Grid KHÔNG phải là mô hình cụm cuối cùng.
# - Cụm cuối vẫn được hình thành bởi granular-ellipsoid + DPC.
# ============================================================
...
class MinMaxNormalizer:
    """
    Chuẩn hóa Min-Max theo từng chiều về [0, 1].

    Vì grid partition chỉ có ý nghĩa khi các chiều cùng thang đo,
    nên bước này được thực hiện trước grid initialization.
    """
    ...
    @classmethod
    def fit(cls, X: np.ndarray) -> "MinMaxNormalizer":
        X = np.asarray(X, dtype=float)
        data_min = np.min(X, axis=0)
        data_max = np.max(X, axis=0)
        data_range = data_max - data_min
        # Tránh chia cho 0 ở các chiều hằng
        data_range[data_range < 1e-12] = 1.0
        return cls(data_min_=data_min, data_range_=data_range)

    def transform(self, X: np.ndarray) -> np.ndarray:
        X = np.asarray(X, dtype=float)
        Xn = (X - self.data_min_) / self.data_range_
        return np.clip(Xn, 0.0, 1.0)

    def fit_transform(self, X: np.ndarray) -> np.ndarray:
        fitted = MinMaxNormalizer.fit(X)
        self.data_min_ = fitted.data_min_
        self.data_range_ = fitted.data_range_
...
class GridCell:
    """
    Một ô lưới không rỗng (non-empty cell).

    key      : tọa độ nguyên của cell trong lưới
    indices  : chỉ số các điểm thuộc cell
    centroid : tâm của các điểm trong cell (ở không gian đã normalize)
    """
...
@dataclass
class LocalSeedRegion:
    """
    Một vùng seed cục bộ được tạo từ nhiều non-empty cells lân cận.

    cell_keys : danh sách key của các cell thuộc cùng một region
    indices   : toàn bộ chỉ số điểm thuộc region
    """
...
# ============================================================
# Grid initialization and local seed generation
# ============================================================
def choose_uniform_grid_size(
    X_norm: np.ndarray,
    target_points_per_cell: int = 16,
    min_bins_per_dim: int = 4,
    max_bins_per_dim: int = 64,
) -> float:
    """
    Chọn kích thước ô lưới đều theo một heuristic đơn giản và minh bạch.

    Thiết kế trong file này:
    - Không sao chép nguyên công thức của IncGridDBC, vì code hiện tại của bạn
      là GE-DPC chứ không phải DBSCAN với tham số epsilon-neighborhood.
    - Ở đây ta chỉ cần grid làm bước phân hoạch thô, nên dùng một lựa chọn ổn định:
      ước lượng số ô mỗi chiều sao cho trung bình mỗi ô có khoảng target_points_per_cell điểm.

    Lý do chọn cách này:
    - Dễ giải thích học thuật.
    - Không làm grid quá mịn hoặc quá thô ngay từ đầu.
    - Không đưa thêm một công thức phụ thuộc trực tiếp vào epsilon hình học của GE.
    """
    n_samples, dim = X_norm.shape
    approx_nonempty = max(1, int(np.ceil(n_samples / max(target_points_per_cell, 1))))
    bins_per_dim = int(np.ceil(approx_nonempty ** (1.0 / max(dim, 1))))
    bins_per_dim = max(min_bins_per_dim, min(max_bins_per_dim, bins_per_dim))
    cell_size = 1.0 / bins_per_dim
    return float(cell_size)


def point_to_grid_key(point: np.ndarray, cell_size: float) -> Tuple[int, ...]:
    """
    Ánh xạ một điểm trong [0,1]^d sang khóa ô lưới.

    Dùng floor(point / cell_size), nhưng khóa lớn nhất được chặn để không tạo
    cell ngoài biên do sai số số thực ở điểm = 1.0.
    """
    point = np.asarray(point, dtype=float)
    bins_per_dim = max(1, int(round(1.0 / cell_size)))
    coords = np.floor(point / cell_size).astype(int)
    coords = np.clip(coords, 0, bins_per_dim - 1)
    return tuple(int(v) for v in coords)


def build_nonempty_grid_cells(X_norm: np.ndarray, cell_size: float) -> Dict[Tuple[int, ...], GridCell]:
    """
    Grid initialization:
    - chia không gian thành uniform grid cells
    - chỉ giữ non-empty cells
    """
    bucket: DefaultDict[Tuple[int, ...], List[int]] = defaultdict(list)
    for idx, x in enumerate(X_norm):
        key = point_to_grid_key(x, cell_size)
        bucket[key].append(idx)

    cells: Dict[Tuple[int, ...], GridCell] = {}
    for key, indices_list in bucket.items():
        indices = np.asarray(indices_list, dtype=int)
        centroid = np.mean(X_norm[indices], axis=0)
        cells[key] = GridCell(key=key, indices=indices, centroid=centroid)
    return cells


def neighboring_keys(key: Tuple[int, ...], radius: int = 1) -> Iterable[Tuple[int, ...]]:
    """
    Sinh các khóa lân cận theo Chebyshev radius.

    Thiết kế trong file này:
    - radius = 1 nghĩa là dùng adjacency trực tiếp trên lưới.
    - Đây là cách đơn giản, đúng tinh thần "gom cell lân cận thành seed cục bộ".
    """
    dim = len(key)

    def rec_build(level: int, current: List[int]) -> Iterable[Tuple[int, ...]]:
        if level == dim:
            yield tuple(current)
            return
        base = key[level]
        for delta in range(-radius, radius + 1):
            ...
            yield from rec_build(level + 1, current)
            current.pop()
...
def build_cell_graph(
    ...
    """
    Xây graph lân cận giữa các non-empty cells.

    Trong phiên bản này, graph dùng để tạo local seed regions,
    không thay thế bước GE clustering ở cuối.
    """
    graph: Dict[Tuple[int, ...], Set[Tuple[int, ...]]] = {key: set() for key in cells.keys()}
    cell_keys = set(cells.keys())

    for key in cells.keys():
        for nb in neighboring_keys(key, radius=adjacency_radius):
            if nb == key:
                continue
            if nb in cell_keys:
                graph[key].add(nb)
...
def connected_components_on_cells(
    graph: Dict[Tuple[int, ...], Set[Tuple[int, ...]]],
) -> List[List[Tuple[int, ...]]]:
    """
    Lấy các connected components trên graph của non-empty cells.

    Mỗi component sẽ là một local seed region sơ bộ.
    """
...
def build_local_seed_regions(
    cells: Dict[Tuple[int, ...], GridCell],
    graph: Dict[Tuple[int, ...], Set[Tuple[int, ...]]],
    min_region_points: int = 2,
) -> List[LocalSeedRegion]:
    """
    Gom các non-empty cells thành local seed regions.

    Rule dùng ở đây:
    - Mỗi connected component của graph cell tạo thành một seed region.
    - Region quá nhỏ (< min_region_points) vẫn được giữ nhưng sẽ được gom riêng thành 1 GE nhỏ.
      Không loại bỏ để tránh mất dữ liệu.
    """
    components = connected_components_on_cells(graph)
    regions: List[LocalSeedRegion] = []

    for comp in components:
        all_indices = np.concatenate([cells[k].indices for k in comp]).astype(int)
        all_indices = np.unique(all_indices)
        if len(all_indices) == 0:
            continue
        # Không xóa vùng nhỏ; chỉ đánh dấu logic bằng comment cho minh bạch.
        regions.append(LocalSeedRegion(cell_keys=list(comp), indices=all_indices))

    # Sắp theo kích thước giảm dần để theo dõi log dễ hơn.
    regions.sort(key=lambda r: r.size, reverse=True)
    return regions


# ============================================================
# Ellipsoid geometry (kế thừa từ code bạn gửi, chỉ giữ logic lõi)
# ============================================================
class Ellipsoid:
    """
    Granular Ellipsoid cho GE-DPC.

    Kế thừa ý tưởng từ code hiện tại của bạn:
    ...
    Điểm mới trong phiên bản file này:
    - Vẫn dùng Cholesky decomposition + cache.
    - Cho phép lưu thêm seed_id để truy vết ellipsoid được sinh từ local seed nào.
      Trường này chỉ để hỗ trợ log/giải thích, không làm thay đổi thuật toán.
    """
...
def splits_ellipsoid(ellipsoid: Ellipsoid, epsilon: Optional[float] = None) -> List[Ellipsoid]:
    ...
    Logic đúng theo phiên bản hiện tại của bạn:
    1) Chia sơ bộ theo khoảng cách Euclidean tới hai đầu mút trục chính.
    2) Gán lại bằng Mahalanobis distance.
    """
    ...
    ell1 = Ellipsoid(cluster1, cluster1_idx, epsilon=eps, seed_id=ellipsoid.seed_id)
    ell2 = Ellipsoid(cluster2, cluster2_idx, epsilon=eps, seed_id=ellipsoid.seed_id)
...
# ============================================================
def ellipse_mahalanobis_distance(
    ellipsoid_i: Ellipsoid,
    ellipsoid_j: Ellipsoid,
...
# ============================================================
# Grid-guided initialization -> Initial ellipsoids
# ============================================================
def generate_initial_ellipsoids_from_grid_seeds(
    X_norm: np.ndarray,
    epsilon: float,
    cell_size: Optional[float] = None,
    target_points_per_cell: int = 16,
    adjacency_radius: int = 1,
) -> Tuple[List[Ellipsoid], Dict[str, object]]:
    """
    Tạo initial ellipsoids từ local seed regions thay vì từ toàn bộ dữ liệu.

    Đây là bước thay đổi lớn nhất so với code gốc của bạn.

    Pipeline con của hàm này:
    1) Grid initialization
    2) Build non-empty cells
    3) Build cell graph
    4) Build local seed regions
    5) Generate one initial ellipsoid per local seed region
    """
    if cell_size is None:
        cell_size = choose_uniform_grid_size(
            X_norm,
            target_points_per_cell=target_points_per_cell,
        )

    cells = build_nonempty_grid_cells(X_norm, cell_size=cell_size)
    graph = build_cell_graph(cells, adjacency_radius=adjacency_radius)
    seed_regions = build_local_seed_regions(cells, graph)

    initial_ellipsoids: List[Ellipsoid] = []
    for seed_id, region in enumerate(seed_regions):
        data_seed = X_norm[region.indices]
        ell = Ellipsoid(data_seed, region.indices, epsilon=epsilon, seed_id=seed_id)
        initial_ellipsoids.append(ell)

    metadata = {
        "cell_size": cell_size,
        "n_nonempty_cells": len(cells),
        "n_seed_regions": len(seed_regions),
        "cells": cells,
        "cell_graph": graph,
        "seed_regions": seed_regions,
    }
    return initial_ellipsoids, metadata


# ============================================================
# Main pipeline
# ============================================================
def run_ge_dpc_grid_seed_cholesky_cache(
    feature_file: Path,
    label_file: Path,
    epsilon: float = 1e-6,
    outlier_t: float = 2.0,
    ...
    """
    Pipeline hoàn chỉnh theo hướng mới:
    Input -> Grid initialization -> Local seed generation
    -> GE generation -> Split/refine -> GE-DPC clustering -> Output
    """
    # --------------------------------------------------------
    # 0) Input
    # --------------------------------------------------------
    raw_data = np.loadtxt(feature_file, dtype=float)
    ...
    if normalize:
        normalizer = MinMaxNormalizer.fit(raw_data)
        data = normalizer.transform(raw_data)
    else:
        data = np.asarray(raw_data, dtype=float)
        normalizer = None

    initial_ellipsoids, grid_meta = generate_initial_ellipsoids_from_grid_seeds(
        data,
        epsilon=epsilon,
    ...
    time_grid = t_grid_end - t_grid_start

    print("=" * 80)
    print("Grid-guided initialization")
    print(f"Data normalized            : {normalize}")
    print(f"Grid cell size             : {grid_meta['cell_size']:.6f}")
    print(f"Number of non-empty cells  : {grid_meta['n_nonempty_cells']}")
    print(f"Number of local seed regions: {grid_meta['n_seed_regions']}")
    print(f"Initial ellipsoid count from seeds: {len(initial_ellipsoids)}")
    print("=" * 80)

    # --------------------------------------------------------
    # 2) GE generation (initial GE from local seeds)
    # --------------------------------------------------------
    t_gen_start = time.time()
    ellipsoid_list: List[Ellipsoid] = list(initial_ellipsoids)

    # num vẫn giữ đúng tinh thần code cũ: safe split threshold = ceil(sqrt(n))
    num = int(np.ceil(np.sqrt(data.shape[0])))
    print(f"Safe split threshold num = ceil(sqrt(n)) = {num}")

    # --------------------------------------------------------
    # 3) Split / refine - safe split loop
    # --------------------------------------------------------
    iteration = 0
    while True:
        iteration += 1
    ...
    # --------------------------------------------------------
    # 4) Split / refine - outlier-detection split
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 5) GE-DPC clustering - attribute computation
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 6) GE-DPC clustering - center selection
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 7) GE-DPC clustering - ellipsoid labeling
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 8) Output mapping - ellipsoid -> point
    # --------------------------------------------------------
    ...
    print(f"0. Grid initialization time (Thời gian khởi tạo lưới): {time_grid:.14f} seconds (giây)")
    ...
    total_valid_time_attr_cluster = time_attr + time_cluster
    total_valid_time_program = time_grid + time_gen + time_attr + time_cluster
    print("-" * 30)
    print(
        f"Total effective runtime (attributes + clustering) "
        f"(Tổng thời gian hiệu dụng: thuộc tính + phân cụm): {total_valid_time_attr_cluster:.14f} seconds (giây)"
    )
    print(
        f"Total effective runtime of the program "
        f"(Tổng thời gian chạy hiệu dụng của chương trình): {total_valid_time_program:.14f} seconds (giây)"
    )
    print("-" * 30)
    ...
    return {
        ...
        "total_valid_time_attr_cluster": total_valid_time_attr_cluster,
        "total_valid_time_program": total_valid_time_program,
        "grid_cell_size": grid_meta["cell_size"],
        "n_nonempty_cells": grid_meta["n_nonempty_cells"],
        "n_seed_regions": grid_meta["n_seed_regions"],
...
# ============================================================
# Dataset registry (giữ lại tinh thần file cũ của bạn)
# ============================================================
...
def run_named_dataset(
    ...
    return run_ge_dpc_grid_seed_cholesky_cache(
...
if __name__ == "__main__":
    ...
    # ========================================================
    # Grid-guided initialization settings
    # --------------------------------------------------------
    # grid_cell_size = None -> tự chọn bằng heuristic
    # target_points_per_cell: mục tiêu số điểm trung bình mỗi cell
    # adjacency_radius = 1 -> chỉ nối các cell kề trực tiếp
    # ========================================================
    normalize = True
    grid_cell_size = None
