...
# ============================================================
# Remove-one Ablation Study for AQG-GE-DPC
# ============================================================
# Mỗi biến thể loại bỏ đúng một thành phần khỏi mô hình đầy đủ:
#   Full AQG-GE-DPC
#   - Quality gate
#   - Distance gate
#   - Cache
...
#
# Quy tắc công bằng:
# - Tất cả biến thể dùng cùng dữ liệu, scaler, k, split_factor và outlier_t.
# - Chỉ một thành phần được tắt ở mỗi dòng.
# - Nhãn thật chỉ dùng ở bước đánh giá ACC/NMI/ARI cuối cùng.
# - Distance gate là kiểm tra hình học thực sự:
#       Dist(GE_L, GE_R) >= tau_distance
#   trong đó Dist là khoảng cách Mahalanobis giữa hai ellipsoid con.
...
def fresh_H_matrix(ell: Ellipsoid) -> np.ndarray:
    """Tính mới H mà không dùng cache."""
    if ell.n_samples <= 1:
...
def mahal_sq_points_ablation(
    ...
    """Mahalanobis point-to-ellipsoid bằng Cholesky; tùy chọn dùng cache."""
    X = np.asarray(points, dtype=float)
    if X.ndim == 1:
        X = X[None, :]
...
def rho_ablation(ell: Ellipsoid, use_cache: bool) -> float:
    if use_cache:
        return float(ell.rho)
    ...
    return float(np.sqrt(np.max(mahal_sq))) if mahal_sq.size else 0.0


def lengths_ablation(ell: Ellipsoid, use_cache: bool) -> np.ndarray:
    if use_cache:
        return ell.lengths

    H = fresh_H_matrix(ell)
    rho = rho_ablation(ell, use_cache=False)
    eigvals = np.linalg.eigvalsh(H)
    eigvals = np.maximum(eigvals, 1e-12)
    return rho * np.sqrt(eigvals)


def density_ablation(ell: Ellipsoid, use_cache: bool) -> float:
    if use_cache:
        return float(ell.density)

    lengths = lengths_ablation(ell, use_cache=False)
...
def split_ellipsoid_ablation(
    ...
    """Tách ellipsoid và tinh chỉnh hai nhóm bằng Mahalanobis."""
    if ell.n_samples <= 1:
        return [ell]
    ...
    d1 = mahal_sq_points_ablation(ell1, data, use_cache=use_cache)
    d2 = mahal_sq_points_ablation(ell2, data, use_cache=use_cache)
    mask1 = d1 < d2
    mask2 = ~mask1
...
def split_until_stable_ablation(
    ...
    """Safe split ban đầu; giống nhau cho mọi biến thể trừ cache."""
...
def ellipse_mahalanobis_distance_ablation(
    ...
    """Khoảng cách Mahalanobis giữa hai ellipsoid."""
    if use_cache:
        avg_H = 0.5 * (ell_i.H_matrix + ell_j.H_matrix)
    else:
...
def distance_gate_accepts(
    child_left: Ellipsoid,
    child_right: Ellipsoid,
    threshold: float,
    use_cache: bool,
) -> bool:
    """
    Cổng khoảng cách hình học.

    Chấp nhận split khi khoảng cách Mahalanobis giữa hai tâm con
    không nhỏ hơn ngưỡng tau_distance.
    """
    separation = ellipse_mahalanobis_distance_ablation(
        child_left,
        child_right,
        use_cache=use_cache,
    )
    return separation >= float(threshold)


def refine_outliers_remove_one(
    initial_ellipsoids: Sequence[Ellipsoid],
    data: np.ndarray,
    ...
    """
    Tinh chỉnh ellipsoid lớn/bất ổn.

    Một split được giữ khi:
    - hợp lệ về kích thước;
    - thỏa quality gate nếu quality gate được bật;
    - thỏa distance gate nếu distance gate được bật.
    """
    ellipsoids = list(initial_ellipsoids)

    for _ in range(max_iterations):
        if not ellipsoids:
            break

        axes_sums = np.asarray(
            [np.sum(lengths_ablation(ell, use_cache=use_cache)) for ell in ellipsoids],
            dtype=float,
        )
        average_axes = float(np.mean(axes_sums))

        outliers = [
            ell
            for ell, axes in zip(ellipsoids, axes_sums)
            if axes > 2.0 * average_axes
        ]
        ...
        normal = [ell for ell in ellipsoids if id(ell) not in outlier_ids]
        refined: List[Ellipsoid] = []

        min_leaf = max(2, int(np.ceil(np.sqrt(data.shape[0]) * 0.1)))

        for parent in outliers:
            ...
            if len(children) != 2 or any(
                child.n_samples < min_leaf for child in children
            ):
            ...
            quality_ok = True
            if use_quality_gate:
                parent_density = density_ablation(parent, use_cache=use_cache)
                children_density = sum(
                    density_ablation(child, use_cache=use_cache)
                    for child in children
                )
                quality_ok = (
                    children_density
                    > float(quality_threshold) * parent_density
                )

            distance_ok = True
            if use_distance_gate:
                distance_ok = distance_gate_accepts(
                    children[0],
                    children[1],
...
def ellipse_distance_ablation(
    ...
    dist_mat = np.zeros((n_ellipsoids, n_ellipsoids), dtype=float)

    for i in range(n_ellipsoids):
        for j in range(i + 1, n_ellipsoids):
            distance = ellipse_mahalanobis_distance_ablation(
                ellipsoids[i],
                ellipsoids[j],
                use_cache=use_cache,
            )
...
def run_remove_one_variant(
    ...
    """Chạy một biến thể remove-one trên một dataset."""
    key = dataset_name.lower()
    registry = get_default_dataset_registry(base_dir)
    ...
    if len(data) != len(true_labels):
        raise ValueError(
            f"Data and label length mismatch for {key}: "
            f"{len(data)} vs {len(true_labels)}"
        )

    data = apply_data_scaler(data, scaler_mode=str(cfg["scaler"]))

    k = int(cfg["k"])
    quality_threshold = float(cfg["outlier_t"])
    split_factor = float(cfg["split_factor"])
    graph_switch_margin = float(cfg.get("graph_switch_margin", 1.75))
    graph_allowed_by_dataset = bool(cfg["allow_graph"])

    # split_factor luôn được giữ cố định giữa các biến thể.
    # Distance gate không còn bị đánh đồng với split_factor.
    num = max(5, int(np.ceil(np.sqrt(data.shape[0]) * split_factor)))

    # 1) Ellipsoid generation
    t_gen_start = time.perf_counter()

    ellipsoids = split_until_stable_ablation(
        data=data,
        num=num,
        epsilon=epsilon,
        use_cache=use_cache,
    ...
    ellipsoids = refine_outliers_remove_one(
        initial_ellipsoids=ellipsoids,
        data=data,
    ...
    densities = np.asarray(
        [density_ablation(ell, use_cache=use_cache) for ell in ellipsoids],
        dtype=float,
    )
    ...
    # Graph correction được kiểm tra khi:
    # - biến thể chưa loại bỏ graph;
    # - cấu hình dataset cho phép graph.
    if use_graph_correction and graph_allowed_by_dataset:
        graph_labels = ellipse_cluster_conservative_graph_correction(
            densities,
            dist_mat,
            centers,
    ...
    # 4) Map labels về từng mẫu
    pred_labels = np.full(len(data), -1, dtype=int)
    for index, ell in enumerate(ellipsoids):
        pred_labels[ell.indices] = ellipsoid_labels[index]

    if np.any(pred_labels == -1):
        raise RuntimeError(
            f"Some samples in {key} were not assigned a label."
        )
    ...
    total_time_ms = (
        time_gen + time_attr + time_cluster
    ) * 1000.0
    ...
    return {
        ...
        "nmi": float(nmi),
        "ari": float(ari),
        "n_ellipsoids": int(len(ellipsoids)),
        "time_ms": float(total_time_ms),
        "mode": label_mode,
        "quality_gate": bool(use_quality_gate),
        "distance_gate": bool(use_distance_gate),
        "cache": bool(use_cache),
        "graph_correction": bool(
            use_graph_correction and graph_allowed_by_dataset
        ),
    }


def run_remove_one_ablation(
    base_dir: Path,
    dataset_names: Sequence[str],
    distance_threshold: float,
    epsilon: float = 1e-6,
) -> List[Dict[str, object]]:
    """Chạy full model và lần lượt loại bỏ từng thành phần."""
    variants = [
        (
            "Full AQG-GE-DPC",
            dict(quality=True, distance=True, cache=True, graph=True),
        ),
        (
            "- Quality gate",
            dict(quality=False, distance=True, cache=True, graph=True),
        ),
        (
            "- Distance gate",
            dict(quality=True, distance=False, cache=True, graph=True),
        ),
        (
            "- Cache",
            dict(quality=True, distance=True, cache=False, graph=True),
        ),
        (
            "- Graph correction",
            dict(quality=True, distance=True, cache=True, graph=False),
        ),
    ]
    ...
    for dataset_name in dataset_names:
        ...
        print("\n" + "=" * 112)
        print(f"REMOVE-ONE ABLATION - {key}")
        print("=" * 112)

        for variant_name, flags in variants:
            row = run_remove_one_variant(
                dataset_name=key,
                base_dir=base_dir,
                variant_name=variant_name,
                use_quality_gate=flags["quality"],
                use_distance_gate=flags["distance"],
                use_cache=flags["cache"],
                use_graph_correction=flags["graph"],
                distance_threshold=distance_threshold,
                epsilon=epsilon,
            )
            results.append(row)
            ...
            print(
                ...
                f"NMI={row['nmi']:.3f}  "
                f"ARI={row['ari']:.3f}  "
                f"Ellips={row['n_ellipsoids']:>4}  "
                f"Time={row['time_ms']:>9.2f} ms  "
                f"Mode={row['mode']}"
            )
    ...
    with output_csv.open("w", newline="", encoding="utf-8") as file:
        ...
        for row in results:
            writer.writerow([
                ...
                row["distance_gate"],
                row["cache"],
                row["graph_correction"],
            ])


def print_remove_one_latex_table(
    results: Sequence[Dict[str, object]],
    dataset_names: Sequence[str],
) -> None:
    print("\n" + "=" * 112)
    print("LATEX TABLE - REMOVE-ONE ABLATION")
    print("=" * 112)

    print(r"\begin{table}[!t]")
    print(r"\centering")
    print(r"\caption{Remove-one ablation study on representative datasets.}")
    print(r"\label{tab:remove_one_ablation}")
    print(r"\scriptsize")
    ...
    for dataset_name in dataset_names:
        ...
        for index, row in enumerate(subset):
            dataset_cell = (
                rf"\multirow{{{len(subset)}}}{{*}}{{{display_name}}}"
                if index == 0
                else ""
            )

            print(
                rf"{dataset_cell} & {row['variant']} "
                rf"& {row['acc']:.3f} "
                rf"& {row['nmi']:.3f} "
                rf"& {row['ari']:.3f} "
                rf"& {row['n_ellipsoids']} "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run a remove-one ablation study for AQG-GE-DPC. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Project base directory containing dataset folders. "
            "Default: parent folder of this file."
        ),
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
        default=["landsat_2", "banknote"],
        help=(
            "Datasets used in the remove-one ablation study. "
            "Use at least one dataset with graph enabled to evaluate "
            "graph correction."
        ),
    )
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Minimum Mahalanobis separation required by the distance gate. "
            "Default: 1.0."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default="remove_one_ablation_results.csv",
    )
...
def main():
    ...
    results = run_remove_one_ablation(
        base_dir=base_dir,
    ...
    write_remove_one_csv(results, output_csv)

    print(f"\nSaved CSV: {output_csv}")
    print_remove_one_latex_table(results, dataset_names)
